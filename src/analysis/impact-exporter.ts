  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim().startsWith('|') && line.trim().endsWith('|')) {
      ...
      if (!inTable) {
        ...
        for (const c of cells) html.push(`<th>${inlineMd(c.trim())}</th>`);
      ...
      html.push('<tr>');
      for (const c of cells) html.push(`<td>${inlineMd(c.trim())}</td>`);
      html.push('</tr>');
      continue;
    } else {
      closeTable();
...
export function generateCodeImpactMarkdown(report: any, filesAnalyzed: string): string {
  const lines: string[] = [];
...
export function generateRequirementPlanMarkdown(report: any): string {
  const lines: string[] = [];
  const now = new Date().toISOString().replace('T', ' ').slice(0, 19);
  ...
  if (report.implementationPlan && report.implementationPlan.length > 0) {
    ...
    for (let i = 0; i < report.implementationPlan.length; i++) {
      ...
      if (step.targetFile) lines.push(`- **Target File:** \`${step.targetFile}\``);
...
export function exportCodeImpact(report: any, filesAnalyzed: string, format: ExportFormat) {
...
export function exportRequirementPlan(report: any, format: ExportFormat) {
